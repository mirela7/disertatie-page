...
function renderGenomePalette() {
  ...
  presets.forEach((preset, index) => {
    ...
    button.innerHTML = `<span class="swatch" style="background:${preset.color}"></span><div><strong>${preset.label}</strong><span>${preset.bits.join('')}</span></div>`;
    button.onclick = () => {
...
function render() {
  if (state.running) {
    const baseSteps = state.gridSize === 256 ? 1 : 2;
